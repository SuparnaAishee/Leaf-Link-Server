...
const getUserPost = catchAsync(async (req, res) => {
  const userId = req.user._id; // Make sure this is a valid ObjectId

  console.log('User ID from Token:', userId); // Log for debugging

  const posts = await postService.getUserPostFromDB(userId); // Pass the user ID to the service
