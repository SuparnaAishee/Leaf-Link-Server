...
export const initiatePayment = async (paymentData: TPayment) => {
  ...
  const res = await axios.post(config.payment_url as string, {
    ...
    success_url: `http://localhost:5000/api/success?transactionId=${paymentData.transactionId}&status=success`,
    fail_url: `http://localhost:5000/api/success?status=failed`,
