...
export const initiatePayment = async (paymentData: TPayment) => {
  ...
  const res = await axios.post(config.payment_url as string, {
    ...
    success_url: `${config.server_base_url}/api/success?transactionId=${paymentData.transactionId}&status=success`,
    fail_url: `${config.server_base_url}/api/success?status=failed`,
